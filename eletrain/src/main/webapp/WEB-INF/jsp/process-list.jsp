...
<title>审批列表</title>
<meta name="description" content="审批列表">
...
<script type="text/javascript">
	...
	// 查看流程详情
	function info(id) {
		window.location.href = "/process/processInfo?appId=" + id + "&menuId=15";
	}
	// 全选
	$(function() {
	...
	$(function() {
		$('#searchBut').click(
				function() {
					...
					window.location.href = "/process/searchByPlanName?planName="
							+ encodeURI(encodeURI(pname))+"&menuId=15";
				});
	});
</script>
				...
				<div class="am-g">
					<div class="am-u-sm-12">
						<table class="am-table am-table-striped am-table-hover table-main">
							...
							<c:forEach varStatus="count" items="${pageBean.list }" var="app">
								<tbody>
									<tr>
										<td><input id="ck" type="checkbox" name="ids"
											value=${app.applicationId }></td>
										<td>${fn:substring(app.title,0,6) }</td>
										<td>${app.username }</td>
										<td class="am-hide-sm-only"><fmt:formatDate
												value="${app.applyDate }" pattern="yyyy-MM-dd" /></td>
											...
											<div class="am-btn-toolbar">
												<div class="am-btn-group am-btn-group-xs">
													<button id="infoBut" type="button"
														onclick="info('${app.applicationId }')"
														class="am-btn am-btn-default am-btn-xs am-hide-sm-only">
														<span class="am-icon-copy"></span>流程详情
													</button>
													<button id="infoBut" type="button" <c:if test="${app.status != 1}">
													    disabled="disabled"</c:if>
														onclick="showWindow('${app.pdId}','${app.applicationId }')"
														class="am-btn am-btn-default am-btn-xs am-hide-sm-only">
														<span class="am-icon-copy"></span>流程进展图
													</button>
													<button onclick="b('${app.planId }')" type="button"
													<c:if test="${app.status == 2}">disabled="disabled"</c:if> 
														class="am-btn am-btn-default js-modal-open">
														<span class="am-icon-archive"></span> 下载方案详情文档
													</button>
													<div class="am-modal am-modal-no-btn" tabindex="-1"
														id="modaldownload${app.planId }">
														<div style="width: 800px;" class="am-modal-dialog">
															...
															<div class="am-modal-bd">
																<div class="am-modal-actions-group">
																	<ul id="downul${app.planId }" class="am-list">
																	</ul>
																</div>
															</div>
														</div>
							...
							<div class="am-fr">
								<ul class="am-pagination tpl-pagination">
									<c:if test="${pageBean.page ne 1}">
										<li><a
											href="${pageContext.request.contextPath }/process/${url }&page=1&menuId=15">首页</a></li>
										<li><a
											href="${pageContext.request.contextPath }/process/${url }&page=${pageBean.page -1}&menuId=15">«</a></li>
									</c:if>
									<c:choose>
									...
									<c:forEach var="i" begin="${begin }" end="${end }">
										...
										<c:if test="${pageBean.page !=  i }">
											<li><a
												href="${ pageContext.request.contextPath }/process/${url }&page=${i}&menuId=15">${i }</a></li>
										</c:if>
									</c:forEach>
									<c:if test="${pageBean.page < pageBean.totalPage  }">
										<li><a
											href="${ pageContext.request.contextPath }/process/${url }&page=${pageBean.page +1}&menuId=15">»</a></li>
										<li><a
											href="${ pageContext.request.contextPath }/process/${url }&page=${pageBean.totalPage}&menuId=15">尾页</a></li>
									</c:if>
								</ul>
							</div>
						</div>
